...
- 日 K 线补特征：DSA 调用 AlphaSift 时会优先复用 DSA 历史行情加载链路（数据库缓存、Tushare、Efinance、Akshare、Pytdx、Baostock、Yfinance 等 fallback），仅在 DSA 链路无可用数据时回退到 AlphaSift 原始日线数据源，减少单一上游超时拖垮选股。例外地，当 AlphaSift 日线源显式为 `tencent` 时，DSA 不再覆盖该链路，而是保留 AlphaSift 原生 Tencent 日线实现，避免与 DSA 历史装载器在最新交易日覆盖和成交量口径上出现契约漂移，导致 `volume_breakout` 等依赖当日突破特征的策略被误清空。
