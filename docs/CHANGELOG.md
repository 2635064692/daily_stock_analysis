...
- [修复] SPI phase2 日终轮动任务会为当日观察池真实抓取并落盘成分股快照，并基于历史 BUY 信号执行可达的 SELL 检查，避免轮动闭环长期只跳过不产信号。
- [修复] 修复 SPI v2 `ema_for_spi_v2` 在恰好等于周期长度时返回空序列导致的边界崩溃，并让 `rotation_entry.yaml` 的观察池/入场/出场参数真正接入运行时。
