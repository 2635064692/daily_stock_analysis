...
| [外部数据源全景](external-data-sources.md) | 行情、搜索、LLM、资讯、通知、社交舆情等全部外部数据源接入方式、配置与 fallback 关系 |
...
| 文档 | 内容 |
| --- | --- |
| [架构总览](ARCHITECTURE.md) | 顶层结构、核心数据流、模块依赖边界 |
| [设计原则](DESIGN.md) | Provider 模式、多 Key 负载均衡、容错降级策略 |
| [搜索 Provider 架构](search-providers.md) | Grok/Bocha/Tavily 等 8 种搜索 Provider 的架构、搜索策略与 fallback 编排 |
