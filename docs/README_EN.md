...
| News Search | [Anspire](https://open.anspire.cn/?share_code=QFBC0FYC), [SerpAPI](https://serpapi.com/baidu-search-api?utm_source=github_daily_stock_analysis), [Tavily](https://tavily.com/), [Grok](https://x.ai/api), [Bocha](https://open.bocha.cn/), [Brave](https://brave.com/search/api/), [MiniMax](https://platform.minimaxi.com/), SearXNG |
...
| `GROK_API_KEYS` | [Grok](https://x.ai/api), real-time web search with LLM-structured output (pair with `GROK_BASE_URL`/`GROK_MODEL`, default `grok-4.20-fast`) | Optional |
