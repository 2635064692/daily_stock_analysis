...
| [External Data Sources](external-data-sources.md) | All external data sources: market data, search, LLM, intelligence, notifications, social sentiment — access methods, configuration, and fallback chains |
...
| Document | Contents |
| --- | --- |
| [Architecture Overview](ARCHITECTURE.md) | Top-level structure, core data flow, module dependency boundaries |
| [Design Principles](DESIGN.md) | Provider pattern, multi-key load balancing, fault tolerance and degradation strategy |
| [Search Provider Architecture](search-providers.md) | Architecture for 8 search providers (Grok, Bocha, Tavily, etc.), search strategies, and fallback orchestration |
