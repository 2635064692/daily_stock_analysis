...
| `GROK_API_KEYS` | Grok 搜索 API Key（实时联网搜索，LLM 结构化返回；配合 `GROK_BASE_URL`/`GROK_MODEL` 使用，默认 `https://api.x.ai/v1` 与 `grok-4.20-fast`） | 可选 |
...
4. **搜索 API**：`GROK_API_KEYS`、`ANSPIRE_API_KEYS` 或 `SERPAPI_API_KEYS`（推荐，用于新闻与舆情搜索）
...
| `GROK_API_KEYS` | Grok 搜索 API Key（实时联网搜索，LLM 结构化返回；多个 key 逗号分隔） | 可选 |
| `GROK_BASE_URL` | Grok API base URL，默认 `https://api.x.ai/v1`，兼容 OpenAI 格式的任意端点 | 默认 `https://api.x.ai/v1` |
| `GROK_MODEL` | Grok 模型名称，默认 `grok-4.20-fast` | 默认 `grok-4.20-fast` |
